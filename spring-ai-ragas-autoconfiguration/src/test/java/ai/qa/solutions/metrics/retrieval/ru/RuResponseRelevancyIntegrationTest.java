...
class RuResponseRelevancyIntegrationTest {
    ...
    public static Stream<Arguments> embeddingModels() {
        return Stream.of(
                ...
                Arguments.of("EmbeddingsGigaR", 1024));
    }
    ...
    // ==================== МЕТРИКА РАБОТАЕТ ХОРОШО (Высокая достоверность) ====================
    ...
    void testResponseRelevancy_PerfectAnswer(String model, int dimensions) {
        ...
        Double score =
                executeTest(sample, "✅ Идеальный ответ: Прямой и полный - ОЖИДАЕТСЯ ВЫСОКИЙ БАЛЛ", model, dimensions);
    ...
    void testResponseRelevancy_NoncommittalAnswer(String model, int dimensions) {
        ...
        Double score = executeTest(sample, "Уклончивый ответ", model, dimensions);
    ...
    void testResponseRelevancy_VerboseButComplete(String model, int dimensions) {
        ...
        Double score = executeTest(sample, "Подробный полный ответ", model, dimensions);
    ...
    void testResponseRelevancy_CompleteVsIncomplete(String model, int dimensions) {
        ...
        Double incompleteScore = executeTest(incompleteSample, "Неполный ответ", model, dimensions);
        Double completeScore = executeTest(completeSample, "Полный ответ", model, dimensions);

        if (completeScore <= incompleteScore) {
            log.error("Модель {} сломана, неполный ответ получает скор лучше, чем полный", model);
        } else {
            assertTrue(
                    true,
    ...
    void testResponseRelevancy_PartialAnswer_UnexpectedlyHigh(String model, int dimensions) {
        if (openAiApi == null && model.contains("/")) return;
        if (openAiApi != null && !model.contains("/")) return;
        ...
        Double score = executeTest(sample, "⚠️ ОГРАНИЧЕНИЕ - Частичный ответ", model, dimensions);
    ...
    void testResponseRelevancy_SameEntity_DifferentAspect(String model, int dimensions) {
        ...
        Double score = executeTest(sample, "⚠️ КРИТИЧЕСКОЕ ОГРАНИЧЕНИЕ - Разный аспект", model, dimensions);
    ...
    void testResponseRelevancy_CompletelyOffTopic(String model, int dimensions) {
        ...
        Double score = executeTest(sample, "⚠️ ОГРАНИЧЕНИЕ - Нерелевантный ответ", model, dimensions);
    ...
    void testResponseRelevancy_DifferentDomains_SimilarStructure(String model, int dimensions) {
        ...
        Double score = executeTest(sample, "⚠️ Нестабильно - Программирование vs Кулинария", model, dimensions);
    ...
    void testResponseRelevancy_SingleWordNonsense(String model, int dimensions) {
        ...
        Double score = executeTest(
                sample, "🚨 Нестабильно и критично - Одно слово не совпадающее с контекстом", model, dimensions);
    ...
    void testResponseRelevancy_ShortQA(String model, int dimensions) {
        ...
        Double score = executeTest(sample, "Граничный случай: Очень короткий Q&A", model, dimensions);
    ...
    void testResponseRelevancy_IncorrectButOnTopic(String model, int dimensions) {
        ...
        Double score = executeTest(sample, "Граничный случай: Неправильный но по теме", model, dimensions);
    ...
    void testResponseRelevancy_RedundantInformation(String model, int dimensions) {
        ...
        Double score = executeTest(sample, "Граничный случай: Ответ с избыточной информацией", model, dimensions);
    ...
    void testResponseRelevancy_HypotheticalQuestion(String model, int dimensions) {
        ...
        Double score = executeTest(sample, "Граничный случай: Гипотетический вопрос", model, dimensions);
    ...
    void testResponseRelevancy_AmbiguousQuestion(String model, int dimensions) {
        ...
        Double score = executeTest(sample, "Граничный случай: Неоднозначный вопрос", model, dimensions);
    ...
    void testResponseRelevancy_ClarificationRequest(String model, int dimensions) {
        ...
        Double score = executeTest(sample, "Граничный случай: Запрос на уточнение", model, dimensions);
    ...
    void testResponseRelevancy_EmptyUserInput(String model, int dimensions) {
        ...
        Double score = executeTest(sample, "Валидация: Пустой ввод пользователя", model, dimensions);
    ...
    void testResponseRelevancy_EmptyResponse(String model, int dimensions) {
        ...
        Double score = executeTest(sample, "Валидация: Пустой ответ", model, dimensions);

        assertEquals(0.0, score, "Пустой ответ должен возвращать 0.0. Получено: " + score);
    }

    /**
     * Выполнить тест со стандартной конфигурацией, красивым логированием и пользовательской меткой
     *
     * @param sample Тестовый образец с вводом пользователя и ответом
     * @param label  Опциональная метка для описания тестового сценария (например, "ОГРАНИЧЕНИЕ", "УСПЕХ")
     * @return Балл релевантности ответа
     */
    @Step("Вызов метрики")
    ...
    /**
     * Получить эмодзи на основе диапазона баллов для быстрой визуальной оценки
     */
    private String getScoreEmoji(double score) {
        if (score == 0.0) return "🔵 (Уклончивый)";
        if (score >= 0.95) return "🟢 (Отлично)";
        if (score >= 0.85) return "🟢 (Очень хорошо)";
        if (score >= 0.75) return "🟡 (Хорошо)";
        if (score >= 0.60) return "🟠 (Средне - Ненадёжно)";
        if (score >= 0.40) return "🔴 (Низко)";
        return "🔴 (Очень низко)";
    }
}
